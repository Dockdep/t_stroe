...
//            'defaultVariant' => DefaultVariantBehavior::className(),
    ...
    public function getEvents(){
        return $this->hasMany(Event::className(), ['id' => 'event_id'])->viaTable('events_to_products', ['product_id' => 'id'])->where(['status'=>Event::ACTIVE]);
    }
...
//            return $this->getOptions()->innerJoinWith('taxGroup')->where([''])
    ...
    public function getFullName():string
    {
        ...
        $groupName = (new Query())->select(['tax_option.*','tax_option_lang.*'])
            ->from([ 'tax_option' ])
            ...
            ->where(['tax_group_to_category.category_id' => $this->category->id, 'tax_group.use_in_name'=>1, 'tax_option.id' => ArrayHelper::getColumn($this->options,'id')])
            ->one();
        ...
        if($this->variant->price != 0 ){
            ...
            if(!$user->isGuest){
                ...
                if($discountCategory != null){
                    $percent = $percent > $discountCategory->discount ? $percent : $discountCategory->discount;
                }
