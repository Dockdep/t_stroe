...
$(document)
    .on('click', '#modal_close, #overlay, #overlay_s, #modal_close-2', function (e) {
    basket.hideBasket();
    $('.forms_, .forms_reg')
        .animate({
        opacity: 0,
...
$(document)
    .on('change', '[name="OrderFrontend[delivery]"]', function (e) {
    if ($(this)
        .val() == 3) {
        $('#payment-wrapper-8')
            .removeClass('hidden');
...
$(document)
    .on('change', '[name="OrderFrontend[payment]"]', function (e) {
    console.log('hello');
